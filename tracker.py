...
def run_mdnet(RGB_img_list,T_img_list, init_bbox, gt=None, seq='seq_name ex)Basketball', 
              savefig_dir='/media/zmcv/data2/4st_micro_light_tracking/4st_tracking/draw_box/triple_component', display=False):
    ...
    tic = time.time()
    # Load first image
    RGB_org_image = Image.open(RGB_img_list[0])
    RGB_org_image = np.asarray(RGB_org_image)

    T_org_image = Image.open(T_img_list[0])
    T_org_image = np.asarray(T_org_image)

    RGB_cur_image = []
    channel_1,channel_2,channel_3= fusion_model.fusion_process(RGB_org_image,T_org_image)
    RGB_cur_image = np.concatenate((channel_1,channel_2,channel_3),2)
    ...
    for bidx in range(0,scene_boxes.shape[0]):
        ...
        feat_map = model(RGB_cropped_image, out_layer='conv3')
    ...
    extra_feat_maps = model(RGB_extra_cropped_image, out_layer='conv3')
    # Draw pos/neg samples
    ishape = RGB_cur_image.shape
    ...
    for i in range(1,len(RGB_img_list)):
        ...
        RGB_org_image = Image.open(RGB_img_list[i])
        RGB_org_image = np.asarray(RGB_org_image)

        T_org_image = Image.open(T_img_list[i])
        T_org_image = np.asarray(T_org_image)
        ...
        # Load image
        # RGB_cur_image = Image.open(RGB_img_list[i])
        # RGB_cur_image = np.asarray(RGB_cur_image)

        # T_cur_image = Image.open(T_img_list[i])
        ...
        if display or savefig:
            ...
            if display:
                ...
                plt.draw()
            if savefig:
                if not os.path.exists(os.path.join(savefig_dir,seq)):
                    os.mkdir(os.path.join(savefig_dir,seq))
                fig.savefig(os.path.join(savefig_dir,seq,'%04d.jpg'%(i)),dpi=dpi)
