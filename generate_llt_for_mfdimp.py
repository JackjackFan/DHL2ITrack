...
def train_mdnet():
    ...
    ## set image directory
    img_home = '/media/zmcv/data21/LOLT156/low-light_infrared_dataset_test/'
    ...
    lolt_mfdim_save_path = "/media/zmcv/data21/LOLT156/low-light_infrared_dataset_test"

    video_list = sorted(os.listdir(img_home))
    
    for i,video_name in enumerate(video_list):
        print('{}---now is processing {}'.format(i,video_name))
        lowlight_img_list = sorted(os.listdir(os.path.join(img_home,video_name,'channel')))
        T_img_list = sorted(os.listdir(os.path.join(img_home,video_name,'channel2')))
        seqlen = len(lowlight_img_list)
        for j,(ll_img_file,t_img_file) in enumerate(zip(lowlight_img_list,T_img_list)):
            ll_img = np.array(Image.open(os.path.join(img_home,video_name,'channel',ll_img_file)))
            t_img = np.array(Image.open(os.path.join(img_home,video_name,'channel2',t_img_file)))          
            channel_1,channel_2,channel_3= fusion_model.fusion_process(ll_img,t_img)
            ...
            channel_1_sava_path = os.path.join(lolt_mfdim_save_path,video_name,'enhance',ll_img_file.split('ll.jpg')[0]+'_enhance.jpg')
            channel_3_sava_path = os.path.join(lolt_mfdim_save_path,video_name,'fusion',ll_img_file.split('ll.jpg')[0]+'_fusion.jpg')
