...
def train_mdnet():
    ...
    ## set image directory
    img_home = '/media/zmcv/VTUAV/test/ST'
    ...
    lolt_mfdim_save_path = "/media/zmcv/data2/VTUAV_test/ST"

    with open(os.path.join(img_home,'vtuav_test_st.txt')) as f:
    ...
    for i,video_name in enumerate(video_list):
        ...
        if i is not 110:
            continue
        # if os.path.exists(os.path.join(lolt_mfdim_save_path,video_name,'fusion')):
        #         continue  

        for j,(ll_img_file,t_img_file) in enumerate(zip(lowlight_img_list,T_img_list)):
            if os.path.exists(os.path.join(lolt_mfdim_save_path,video_name,'enhance',ll_img_file)):
                print('this img is exists')
                continue
            ...
            if not os.path.exists(os.path.join(lolt_mfdim_save_path,video_name.split('/')[0])):
                os.mkdir(os.path.join(lolt_mfdim_save_path,video_name.split('/')[0]))

            if not os.path.exists(os.path.join(lolt_mfdim_save_path,video_name)):
                os.mkdir(os.path.join(lolt_mfdim_save_path,video_name))                
            ...
            if not os.path.exists(os.path.join(lolt_mfdim_save_path,video_name,'fusion')):
                os.mkdir(os.path.join(lolt_mfdim_save_path,video_name,'fusion'))                                

            channel_1_sava_path = os.path.join(lolt_mfdim_save_path,video_name,'enhance',ll_img_file)
            channel_3_sava_path = os.path.join(lolt_mfdim_save_path,video_name,'fusion',t_img_file)
